        ...
        <article class="question-card ${q[CONFIG.API_FIELDS.SOLVED] ? "solved-card" : ""}">
          ...
          <div class="question-actions">
            <span>
              ...
              <a href="#" class="read-more" data-id="${q.id}">See answers</a>
            </span>
            ${
	...
	try {
      ...
      <article class="question-card question-detail">
        ...
        ${q.imageUrl ? `<img class="question-image" src="${q.imageUrl}" alt="">` : ""}
				<div>
					${q.answers.map((a) => `<p class="question-answer">${a}</p>`).join("")}
				</div>
...
async function showQuestionForm(qId) {
	const container = document.getElementById("questions-container");
	const isEdit = !!qId;
	let q = { question: "", answers: [], keywords: [] };
    ...
    <div class="question-form-wrapper">
      ...
      <form id="question-form" enctype="multipart/form-data">
        ...
        <div class="form-group">
          ...
          <textarea id="q-answer" rows="4" value="${q.answers.join("\n")}" required>${q.answers.join("\n")}</textarea>
        </div>
        <div class="form-group">
	...
	document
		...
		.addEventListener("submit", async (e) => {
			...
			body.append("answers", document.getElementById("q-answer").value);
					...
					if (result.correct) {
						...
						resultEl.innerHTML = `
								<div class="play-result incorrect">
									Incorrect! The answer${correctAnswers.length > 1 ? "s were" : " was"}:
